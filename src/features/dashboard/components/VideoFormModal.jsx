...
const VideoFormModal = ({
  ...
  const uploadFile = async ({ bucket, file, videoId, column, pathColumn }) => {
    ...
    const { publicUrl } = await uploadService.uploadFile({ bucket, path: filePath, file });
    ...
    >
      ...
      <form id="video-form" onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Input label={tr('video-form-title-en')} name="title_en" value={formData.title_en} onChange={handleInputChange} required />

          <Input label={tr('video-form-title-ar')} name="title_ar" value={formData.title_ar} onChange={handleInputChange} required />

        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">

          <Textarea label={tr('video-form-desc-en')} name="description_en" value={formData.description_en} onChange={handleInputChange} rows={4} />

          <Textarea label={tr('video-form-desc-ar')} name="description_ar" value={formData.description_ar} onChange={handleInputChange} rows={4} />
            ...
            preview={
              ...
              video?.thumbnail_url && !thumbnailFile ? (
                ...
                <div className="mt-3">
                  ...
                  <p className="text-xs text-[var(--color-text-muted)] mb-2">{tr('video-form-thumbnail-current')}</p>

                  <img src={video.thumbnail_url} alt="Thumbnail" className="w-24 h-24 object-cover rounded-lg border border-[var(--color-border)]" />
