    ...
    <section id="programs" className="section-py relative overflow-hidden">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <p className="text-[var(--color-text-muted)]">{t('programs.subtitle')}</p>
        </div>
        ...
        {isLoading ? (
          ...
          <p className="text-center text-[var(--color-text-muted)]">{t('programs.empty')}</p>
        ) : (
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {data.map((program) => {
              ...
              return (
                <div key={program.id} className="bg-[var(--color-surface)] rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all hover:-translate-y-1">
                  <OptimizedImage
                    src={imageSrc}
