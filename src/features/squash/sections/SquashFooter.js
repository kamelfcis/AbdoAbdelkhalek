      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div>
            ...
            <p className="text-[var(--color-text-muted)]">{t('footer.tagline')}</p>
          </div>
          <div>
            ...
            <ul className="space-y-2">
              {links.map((link) => (
                <li key={link.href}>
                  <a href={link.href} className="text-[var(--color-text-muted)] hover:text-[var(--color-text-inverse)] transition">
                    {link.label}
                  </a>
                </li>
              ))}
