...
const SquashFAQ = () => {
  ...
  return (
    <section id="faq" className="section-py relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-[var(--color-primary-light)]/10 via-[var(--color-bg)]/80 to-[var(--color-bg-muted)]/90" style={{ zIndex: 1 }} />
      ...
      <div className="container mx-auto px-4 relative" style={{ zIndex: 2 }}>
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-[var(--color-text)] max-w-3xl mx-auto">{t('faq.subtitle')}</p>
        </div>

        {isLoading ? (
          <div className="max-w-4xl mx-auto space-y-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="bg-[var(--color-surface)] rounded-2xl h-16 animate-pulse" />
            ))}
          </div>
        ) : error ? (
          <p className="text-center text-red-600">{t('common.error')}</p>
        ) : faqs.length === 0 ? (
          <p className="text-center text-[var(--color-text-muted)]">{t('faq.empty')}</p>
        ) : (
          <div className="max-w-4xl mx-auto space-y-4">
            {faqs.map((faq, index) => (
              <div key={faq.id} className="bg-[var(--color-surface)]/90 backdrop-blur-md rounded-2xl shadow-lg overflow-hidden border border-transparent hover:border-[var(--color-primary)]/30">
                <button
                  type="button"
                ...
                >
                  ...
                  >
                    <p className={`text-[var(--color-text)] leading-relaxed pt-2 border-t border-[var(--color-primary-light)]/30 ${isRTL ? 'text-right' : 'text-left'}`}>
