...
// Professional lazy loading with webpack chunk names for better code splitting
// Group related components into chunks for optimal loading
const Categories = lazy(() => 
  import(/* webpackChunkName: "content-components" */ './components/Categories')
);
const Videos = lazy(() => 
  import(/* webpackChunkName: "content-components" */ './components/Videos')
);
const Packages = lazy(() => 
  import(/* webpackChunkName: "content-components" */ './components/Packages')
);
const AboutMe = lazy(() => 
  import(/* webpackChunkName: "about-components" */ './components/About')
);
const AboutCoach = lazy(() => 
  import(/* webpackChunkName: "about-components" */ './components/AboutCoach')
);
const WhyChooseMe = lazy(() => 
  import(/* webpackChunkName: "about-components" */ './components/WhyChooseMe')
);
const SuccessStories = lazy(() => 
  import(/* webpackChunkName: "social-components" */ './components/SuccessStories')
);
const Reviews = lazy(() => 
  import(/* webpackChunkName: "social-components" */ './components/Reviews')
);
const FAQ = lazy(() => 
  import(/* webpackChunkName: "support-components" */ './components/FAQ')
);
const Contact = lazy(() => 
  import(/* webpackChunkName: "support-components" */ './components/Contact')
);

// Professional loading fallback with accessibility
const ComponentLoader = ({ message }) => (
  <div 
    className="flex items-center justify-center py-20 min-h-[200px]" 
    role="status" 
    aria-live="polite"
    aria-label={message || "Loading content"}
  >
    <div className="rounded-full h-8 w-8 border-2 border-gray-200 border-t-[#0074b7] animate-spin" aria-hidden="true"></div>
    {message && (
...
function Home() {
  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      try {
        // Defer session check significantly to avoid blocking
        setTimeout(async () => {
          try {
    ...
    // Defer initialization until after page is fully interactive
    if (document.readyState === 'complete') {
      setTimeout(initializeApp, 1000);
    } else {
      window.addEventListener('load', () => setTimeout(initializeApp, 1000), { once: true });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchProfileDetails = async () => {
      ...
      try {
        ...
        const [{ data: userData, error: userError }, { data: videoAccess, error: videoError }, { data: categoryAccess, error: categoryError }, { data: subscriptions, error: subscriptionError }] =
          await Promise.all([
            supabase
              .from('users')
              .select('full_name, email, phone, created_at, is_coach')
              .eq('id', userId)
              .single(),
            supabase.from('user_video_access').select('video_id').eq('user_id', userId),
            supabase.from('user_category_access').select('category_id').eq('user_id', userId),
            supabase
              .from('subscriptions')
              .select(
                `
                  ...
                  end_date,
                  created_at,
                  packages!inner(id, name_en, name_ar, price_egp, price_usd, duration_days)
                `
              )
              .eq('user_id', userId)
        ...
        if (userError || videoError || categoryError || subscriptionError) {
          throw userError || videoError || categoryError || subscriptionError;
        }
        ...
        if (isMounted) {
          setProfileDetails({
            ...
            videoCount: videoAccess?.length || 0,
            categoryCount: categoryAccess?.length || 0,
  ...
  const handleLogout = async () => {
    try {
      ...
      showAlert(currentLanguage === 'ar' ? 'فشل تسجيل الخروج' : 'Failed to logout');
    }
  };
    ...
    <div className="App font-['Open_Sans',_sans-serif] bg-white scroll-smooth" role="main">
      ...
      <main>
        ...
        <ErrorBoundary fallbackTitle="Packages Section" fallbackMessage="Unable to load packages. Please refresh the page.">
          ...
          <Packages onAlert={showAlert} userSession={userSession} />
        </Suspense>
        </ErrorBoundary>
        ...
        >
          ...
          >
            <div className="flex justify-between items-center p-4 border-b">
              ...
              <button
                className="text-gray-600 hover:text-[#0074b7] text-2xl"
                onClick={() => setShowProfileModal(false)}
            ...
            <div className="p-6 space-y-6">
              {profileDetails.loading ? (
                <div className="text-center py-10">
                  <div className="inline-block rounded-full h-10 w-10 border-4 border-gray-200 border-t-[#0074b7] animate-spin mb-4"></div>
                ...
                <>
                  <div className="text-center mb-6">
                    <div className="w-20 h-20 rounded-full bg-gradient-to-r from-[#bfd7ed] to-[#0074b7] flex items-center justify-center mx-auto mb-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <h3 className="font-semibold text-gray-800 mb-2">
                        <i className="fas fa-info-circle mr-2 text-[#0074b7]"></i>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <h3 className="font-semibold text-gray-800 mb-2">
                        <i className="fas fa-video mr-2 text-[#0074b7]"></i>
                  ...
                  {profileDetails.subscriptions.length > 0 && (
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <h3 className="font-semibold text-gray-800 mb-3 flex items-center">
                        <i className="fas fa-user-check mr-2 text-[#0074b7]"></i>
                        {currentLanguage === 'ar' ? 'الاشتراكات النشطة' : 'Active Subscriptions'}
                      </h3>
                  ...
                  <div className="p-4 bg-blue-50 rounded-lg">
                    <h3 className="font-semibold text-gray-800 mb-2 flex items-center">
                      <i className="fas fa-star mr-2 text-[#0074b7]"></i>
